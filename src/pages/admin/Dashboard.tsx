  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const res = await fetch(`${ADMIN_API_URL}/api/carparks/stats`, {
    ...
    return (
      <AdminLayout title="Dashboard">
        <div className="flex items-center justify-center h-64">
          <p className="text-on-surface-variant animate-pulse font-bold uppercase tracking-widest text-sm">Loading dashboard...</p>
        </div>
      </AdminLayout>
    );
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6">
          <div className="bg-surface-container-lowest p-6 rounded-lg relative overflow-hidden group editorial-shadow">
            ...
            <div className="flex items-baseline space-x-2">
              <h4 className="text-5xl font-extrabold text-primary leading-none">{stats?.activeSessions ?? 0}</h4>
            ...
            <div className="mt-6 flex -space-x-2">
              ...
              <div className="w-6 h-6 rounded-full border-2 border-surface-container-lowest bg-surface-container-highest text-[8px] flex items-center justify-center font-bold">+{stats?.totalCarparks ?? 0}</div>
            </div>
          </div>

          <Link to="/admin/complaints" className="bg-[#3e0502] p-8 rounded-lg relative overflow-hidden shadow-xl group hover:scale-[1.02] transition-transform">
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              <div className="relative w-48 h-48 mb-8">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-2xl font-black">{stats?.utilization ?? 0}%</span>
