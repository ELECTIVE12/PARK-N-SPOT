...
function ReportsScreen() {
  ...
  const handleExportPDF = () => {
    const content = [
      "PARK 'N SPOT - ANALYTICS REPORT",
      `Period: ${dateRange.start} to ${dateRange.end}`,
      `Generated on: ${new Date().toLocaleString()}`,
      "",
      "=== Facility Overview ===",
      `Total Facility Utilization: ${stats?.utilization}%`,
      `Available Slots: ${stats?.availableSlots}`,
      `Active Sessions: ${stats?.activeSessions}`,
      `Daily Incidents: ${stats?.dailyIncidents}`,
      `Avg. Response Time: ${stats?.avgResponse}`,
      `Customer Satisfaction: ${stats?.satisfiedRate}`,
      "",
      "=== Occupancy by Zone ===",
      ...occupancyData.map(z => `${z.id}\t${z.name}\t${z.location}\t${z.value}%`),
      "",
      "=== Recent Activity Log ===",
      ...activityLog.map(l => `${l.name}\t${l.location}`)
    ].join('\n');
    ...
    a.href = url;
    a.download = `park-n-spot-report-${dateRange.start}-to-${dateRange.end}.txt`;
    a.click();
  ...
  const handleDateChange = (start: string, end: string) => {
    ...
    // Simulate data shift based on date range
    fetch("/api/stats")
      .then(res => res.json())
      .then(data => {
        const seed = start.split('-').reduce((acc, val) => acc + parseInt(val), 0);
        const shift = (seed % 10) - 5; // -5 to +4
        setStats({
          ...data,
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white/80 backdrop-blur-md p-4 rounded-lg editorial-shadow mb-4 lg:mb-8 relative z-30 gap-4">
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className="bg-surface-container-highest text-on-surface px-4 py-2 rounded-sm text-xs font-bold flex items-center gap-2 hover:bg-surface-container-high transition-all border border-outline-variant/30"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="md:col-span-3 bg-surface-container-lowest rounded-xl p-8 border-l-4 border-primary editorial-shadow">
          ...
          <div className="h-64 w-full bg-surface-container-low/50 rounded-lg overflow-hidden relative">
            ...
            <div className="absolute bottom-4 left-0 right-0 flex justify-between px-8 text-[10px] font-bold text-on-surface-variant/40 uppercase tracking-tighter">
