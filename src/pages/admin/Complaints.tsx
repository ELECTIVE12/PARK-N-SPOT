...
import { Complaint, StatData } from "../../types";
        ...
        <div className="flex items-center justify-between border-b border-outline-variant/10 overflow-x-auto pb-1">
          ...
          >
            {({ blob, url, loading, error }) => (
              <button 
                disabled={loading}
                className="mb-2 ml-4 bg-surface-container-highest text-on-surface px-3 py-1.5 rounded-sm text-[10px] lg:text-xs font-bold flex items-center gap-2 hover:bg-surface-container-high transition-all border border-outline-variant/30 shrink-0 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
